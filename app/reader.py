...
from utils import raise_error, DialogLayout
...
class Reader:
    ...
    def init_reader(self, layout, serial, samples_data, filename, pause_event, digits_after_dec=3, logging=False):
    ...
    def run(self, layout, serial, calculation_data, samples_data, filename, pause_event, period=1, runtime=None,
                  digits_after_dec=3, logging=False):

        self.init_reader(layout=layout, serial=serial, samples_data=samples_data, filename=filename, pause_event=pause_event, digits_after_dec=3, logging=False)
        ...
        time_elapsed = self.layout.time_elapsed.get()
        while not getattr(self.thread, "stop_thread", False) and (runtime is None or time_elapsed <= runtime):
            if getattr(self.thread, "pause", False):
                self.pause_event.wait()
            start = time.time()
            self.layout.time_elapsed.set(time_elapsed)
            if self.collect_samples:
                self.update_samples_layout_data(time_elapsed)
            if getattr(self.thread, "start_new_sample", False):
                setattr(self.thread, "start_new_sample", False)
                self.handle_start_new_sample_command(time_elapsed)
            if getattr(self.thread, "start_new_sample_pack", False):
                setattr(self.thread, "start_new_sample_pack", False)
                self.hande_start_new_sample_pack_command()
            reading = self.get_reading(serial, logging=logging)
            passed_validation = False
            if reading is not None:
                if self.validate_reading(reading, calculation_data,
                                         last_reading=self.last_entry.get('mass',0)  if self.last_entry else 0):
                    passed_validation = True
                    Thread(target=self.handle,
                           args=(layout, time_elapsed, reading,
                                 calculation_data, self.last_reading, digits_after_dec)).start()
                    self.last_reading = self.last_read.get('mass',0)
            if time_elapsed % period == 0:
                self.prepare_reading(passed_validation, reading,time_elapsed, self.last_reading, calculation_data)
    ...
    def handle(self, layout, now, reading, calculation_data, last_reading=None, digits_after_dec=2):
        ...
        self.last_read = {
            'total_mass': self.last_read.get('total_mass', 0) + mass_difference,
            'mass': mass,
            'mass_difference': mass_difference,
            'current_mass_difference': mass_difference,
            'volume': volume,
            'flow': flow,
        ...
        if self.interpolation_data:
            ...
            (inter_mass, inter_diff, inter_volume,
             inter_flow, inter_perm) = self.calculate(self.interpolation_data['reading'], calculation_data,
                                                      last_reading=last_entry.get('mass',0) ,
                                                      interpolate=True,
                                                      prev_diff=last_entry.get('mass_difference',0),
                                                      next_diff=mass_difference)
            inter_total_mass = last_entry.get('total_mass', 0) + inter_diff
            self.write_reading(self.interpolation_data['time'], inter_total_mass,  inter_mass, inter_diff, inter_flow, inter_perm)
    ...
    def prepare_reading(self, passed_validation, reading,time_elapsed, last_reading, calculation_data):
        if passed_validation:
            self.handle_interpolation(self.layout, reading,
                                      calculation_data, self.last_entry)
            mass, mass_difference, volume, flow, permeability = self.calculate(self.last_read['mass'],
                                                                               calculation_data,
                                                                               last_reading=
                                                                               self.last_entry.get('mass', 0)
                                                                                 if self.last_entry else None)
            entry = {
                'total_mass': self.last_entry.get('total_mass',0) + mass_difference,
                'mass': mass,
                'mass_difference': mass_difference,
                'flow': flow,
                'volume': volume,
            ...
            self.write_reading(time_elapsed, entry['total_mass'], entry['mass'], entry['mass_difference'], entry['flow'],
                               entry['permeability'])
            self.last_entry = entry
            self.layout.entries_made.set(self.layout.entries_made.get() + 1)
        else:
    ...
    def write_reading(self, current_time, total_mass, mass, mass_difference, flow, permeability):
        with open(self.filename, "a+", encoding="utf-8") as file:
            file.write(
                f"{current_time}  {total_mass:.{self.digits_after_dec}f}  {mass_difference:.{self.digits_after_dec}f}  {flow:.{self.digits_after_dec}f}  {permeability:.{self.digits_after_dec}f}\n")
    ...
    def new_sample_pack(self):
        self.last_entry = {
            'total_mass': self.last_entry.get('total_mass', 0)
        }
        self.last_read = {}
        self.sample_data['current_volume'] = 0
        self.last_reading = 0


    def handle_samples(self, time_elapsed):
        self.sample_data['current_volume'] += self.last_read.get('current_mass_difference', 0)
        if not self.sample_data['continue_sample']:
            if self.sample_data['current_volume'] >= self.sample_data['max_volume']:
                Thread(target=self.handle_sample_max_volume_reached).start()

    def hande_start_new_sample_pack_command(self):
        self.new_sample_pack()

    def handle_start_new_sample_command(self, time_elapsed):
        self.new_sample(time_elapsed)

    def handle_sample_max_volume_reached(self):
        self.sample_data['continue_sample'] = True

        DialogLayout(self.layout.root, self,
                     title="Достигнут максимальный объем пробы",
                     message=f"Достигнут максимальный объем {self.sample_data['max_volume']} мл для пробы {self.sample_data['current_sample']}\nПерейти к следующей пробе?",
                     command1=self.layout.new_sample,
                     command2=self.layout.new_sample_pack,
                     command3=None,
                     buttontext1='Да',
